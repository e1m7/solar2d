...
-- Создание сцен
-- 1) меню = заголовок
-- 2) игра = игровая сцена
-- 3) рекорды = список рекордов
-- Для создания сцен есть элемент Composer
-- Каждая сцена = отдельный lua-файл 

-- 1) main.lua => main_original.lu
-- 2) scene_template.lua => создать заготовку для будущих сцен
-- 3) создать и отредактировать main.lua (настройки + переход к меню)
-- 4) cоздать и отредактировать menu.lua (фон + заголовок + 2 текстовых объекта-кнопки)
